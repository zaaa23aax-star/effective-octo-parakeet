...
if st.button("🚀 Generate", type="primary", use_container_width=True):
    if not prompt.strip():
        ...
        if "error" in result and backup_model != "None":
            st.warning(f"⚠️ Primary model failed: {result['error']}")
            st.info(f"🔄 Trying backup model: {backup_model}")
            
            with st.spinner(f"🧠 Generating with {backup_model}..."):
                result = call_openrouter(backup_model, prompt, OPENROUTER_API_KEY)
        
        if "error" in result:
            st.error(f"❌ Error: {result['error']}")
            
            with st.expander("🔍 Debug Info"):
                st.code(f"Primary Model: {selected_model}\nBackup Model: {backup_model}\nKey: {OPENROUTER_API_KEY[:20]}...")
                st.markdown("**Try:**")
                st.markdown("- Use a different model (try openai/gpt-3.5-turbo)")
                st.markdown("- Check credits at https://openrouter.ai/credits")
                st.markdown("- Reset your keys and try again")
                st.markdown("- Verify model access (some models require approval)")
        else:
            # Display AI response
